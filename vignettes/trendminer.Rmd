...
## Fetching and visualzing time series data of tags

All functions and examples we covered so far dealt with extracting and visualizing asset framework data. In this section we will move from the asset framework API functions to functions which focus on the TrendMiner time series API for tags. 

In the examples below we will focus on the set of tags which are part of the Reactor 1 sub asset tree of site Barcelona which we already know from above:

```{r}
reactor1_str <- tm_af_child_structures(token,"5a0d1bf8-f298-4946-b523-8dcd5194fbaf")
reactor1_str %>% 
  select(parentName, name, tagName, type )
```

`tm_ts_interpolated_data()` lets you retrieve time series data by tag name. The start and the end time of the time series you like to fetch need to be defined as POSIXct objects having set "UTC" as the timezone:
...
Below we are using `ggplot2` to visualize the time series data we just downoladed for tag `r ba_conc1_data$tag$tagName`:
...
all_data <- map(tags, ~tm_ts_interpolated_data(token, .x,  start, end, 2))

 
tags2 <- map_chr(all_data, ~.[["tag"]][["tagName"]])
dfs <- map(all_data, ~.[["timeSeries"]])

dfs <- map2_dfr(tags2, dfs, 
                ~.y %>% 
    mutate(
      tag_name = .x
    ))
