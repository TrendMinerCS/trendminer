...
We start by visualizing the time series data we just downoladed for tag `r ba_conc1_data$tag$tagName`:
...
If you know that the tags you are interested in operate on the same scale you can fetch and visualize their data in one go my chaining several ``purrr::map()` calls together. Below, however, we will break the data fetching and visualing process into two steps to explain some of the underlying details.
