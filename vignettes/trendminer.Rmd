...
* Searching, browsing and visualizing the asset hierarchy
...
## Visualizing the asset hierachy 
...
all_sites_list <- map(roots$structureId, tm_descendant_structures, token = token)
all_site_trees <- map(all_sites_list, FromDataFrameTable, pathName = "externalId")
...
```
```{r}
walk(all_site_trees, function(x) {
  nw <- diagonalNetwork(ToListExplicit(x, unname = TRUE))
  print(nw)
}) 
